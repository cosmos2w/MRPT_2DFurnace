...
class Mutual_SensorToFeature_InterInference(nn.Module):
    def __init__(self, layer_sizes, PreTrained_net, num_fields = 9):
        ...
        self.MLPs = nn.ModuleList([MLP(layer_sizes) for _ in range(num_fields)]) # The field_idx(th) mlp is used to map from the merged_unify to final output 
    ...
    def forward(self, Yin, Gin, field_idx, num_heads, min_val_all, max_val_all):
        
        Base_Y = self.PosNet(Yin)
        # compressed_info = self._compress_data(Base_Y, Gin, num_heads)

        Gin_Y =  torch.cat((Base_Y, Gin), dim=2)
        base_for_Gin_Y = self.net_Y_Gins[field_idx](Gin_Y)
        field_info = self.transformer_layers[field_idx](base_for_Gin_Y, field_idx, num_heads)
        # field_info = self.final_norm(field_info)
        compressed_info = field_info.mean(dim=1) #  This is the approximate field latent feature generated by the pre-trained encoder from sparse measurements

        # Normalize compressed_info
        Norm_compressed_info = self.normalize_data(compressed_info, min_val_all[field_idx], max_val_all[field_idx])

        Predicted_Features_list = []
        for id in range(self.n_fields):
            if id == field_idx:
                Feature_id = Norm_compressed_info
            else:
                Feature_id = self.MLPs[id](Norm_compressed_info)
        ...
        for id in range(self.n_fields):
            if id == field_idx:
                DeNorm_Feature_id = compressed_info
            else:
        ...
        U_Unified_Merged = self.FinalMerge(DeNorm_Predicted_Features)
        Norm_U_Unified = self.normalize_data(U_Unified_Merged, min_val_all[self.n_fields], max_val_all[self.n_fields])

        # U_Unified = self.MLPs[field_idx](Norm_compressed_info)  # The field_idx(th) mlp is used to map from the merged_unify to final output 
        U_Unified = self.MLPs[field_idx](Norm_U_Unified)  # The field_idx(th) mlp is used to map from the merged_unify to final output 
    
        return Predicted_Features, U_Unified

# Using F2F inter-inference to recover the unified latent feature from single-field sparse measurements
class New_Mutual_SensorToFeature_InterInference(nn.Module):
    def __init__(self, layer_sizes, PreTrained_net, num_fields = 9):
        super(New_Mutual_SensorToFeature_InterInference, self).__init__()
        self.n_fields = num_fields
