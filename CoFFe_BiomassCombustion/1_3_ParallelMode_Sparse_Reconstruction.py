...
from network import CoFFe_PreTrain_Net_ParallelMode, Finetuning_SensorToFeatures_ParallelMode, Direct_SensorToFeature
...
N_REPORT       = 10
EVALUATE       = True
hidden_sizes   = [300, 300] # The size of hidden layers for the fine-tuning MLPs
...
NET_TYPE = int(0) 
                # 0 = [CoFFe_PreTrain_Net_ParallelMode]; 1 = [Direct_SensorToFeature]
...
NET_NAME = [f'CoFFe_Parallel_SensorToFeature_FromF{field_idx}_N{N_selected}', f'Direct_SensorToFeature_N{N_selected}']
...
if __name__ == '__main__':
    ...
    with open(f'data_split/CoFFE_Parallel_Features_From{field_idx}_{N_selected}.pic', 'rb') as fp:
    ...
    # Define the network
    if (NET_TYPE == 0):
        net = Finetuning_SensorToFeatures_ParallelMode(layer_sizes, Final_layer_sizes, PreTrained_net).to(device)
        ...
        for U, Y, G, Yin, Gin in get_data_iter(U_train, Y_train, G_train, Yin_train, Gin_train):
            ...
            if EVALUATE is True and (epoch + 1) % N_REPORT == 0:
                with torch.no_grad():
                    ...
                    field_loss_data, field_losses = field_mse_loss(Global_Unified_Gout, G, field_weights)
