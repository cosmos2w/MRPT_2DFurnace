...
NET_NAME     = f'CoFFe_MutualDecodingMode'
...
if __name__ == '__main__':
    ...
    for field_idx in range(len(field_names) + 1):
        ...
        with open(f'Loss_csv/train_test_loss_MutualDecoding/train_test_loss_MultiField_{NET_NAME}_{field_name_or_feature}.csv', 'wt') as fp: 
            pass
    ...
    net = CoFFe_PreTrain_Net_MutualDecodingMode(n_field_info, n_baseF, num_heads, num_layers, num_fields=len(field_names)).to(device)
    ...
    for epoch in range(N_EPOCH):
        ...
        if (epoch + 1) % 20 == 0:
            ...
            for field_idx in range(len(field_names) + 1):
                ...
                field_name_or_feature = 'Unified_feature' if field_idx == len(field_names) else field_names[field_idx]
                with open(f'Loss_csv/train_test_loss_MutualDecoding/train_test_loss_MultiField_{NET_NAME}_{field_name_or_feature}.csv', 'at', newline='') as fp:
