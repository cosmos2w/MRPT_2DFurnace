...
NET_NAME     = f'CoFFe_MutualEncodingMode'
...
if __name__ == '__main__':
    ...
    for field_idx in range(len(field_names)):   # Saving losses to reconstruct the field_idx(th) field from [all fields & Unifed]
        with open(f'Loss_csv/train_test_loss_MutualEncoding/train_test_loss_{NET_NAME}_For_{field_names[field_idx]}.csv', 'wt') as fp:
            pass
    with open(f'Loss_csv/train_test_loss_MutualEncoding/train_test_loss_{NET_NAME}_GlobalUnifed.csv', 'wt') as fp:
        pass
    ...
    field_weights_default = torch.tensor([1.0] * 9)  # Replace with modified weights if needed
    field_weights_default = field_weights_default.to(device)

    net = CoFFe_PreTrain_Net_MutualEncodingMode(n_field_info, n_baseF, num_heads, num_layers, num_fields=len(field_names)).to(device)
    ...
    for epoch in range(N_EPOCH):
        ...
        if (epoch + 1) % PRINT_NUM == 0:
            ...
            for field_idx in range(len(field_names)):
                ...
                with open(f'Loss_csv/train_test_loss_MutualEncoding/train_test_loss_{NET_NAME}_For_{field_names[field_idx]}.csv', 'at', newline='') as fp:
            ...
            with open(f'Loss_csv/train_test_loss_MutualEncoding/train_test_loss_{NET_NAME}_GlobalUnifed.csv', 'at', newline='') as fp:
                writer = csv.writer(fp, delimiter='\t')
                if ((epoch + 1) // PRINT_NUM == 1):
                    fp.write(NET_SETTINGS)
                    header = ['Epoch', 'Overall_Train_Loss', 'Overall_Test_Loss']
