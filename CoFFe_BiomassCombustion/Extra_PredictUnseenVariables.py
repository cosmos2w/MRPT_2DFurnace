...
from network import CoFFe_PreTrain_Net_MutualEncodingMode, UNSeen_FineTune_Net, Finetuning_SensorToFeatures_MutualEncoding
...
mode = True         #   True: finetune based on the pre-trained model / False: Using a newly trained DeepONet
...
NET_NAME = f'CoFFe_LearnUnseen_{mode}'

PreTrained_Net_Name = 'net_CoFFe_MutualEncodingMode_state_dict'
Load_file_path      = f'Output_Net/{PreTrained_Net_Name}.pth'

P2F_Net_Name      = 'net_CoFFe_MutualEncoding_SensorToFeature_FromF0_N25_state_dict'
Load_P2F_Net_path = f'Output_Net/{P2F_Net_Name}.pth'
...
if __name__ == '__main__':
    ...
    PreTrained_net = CoFFe_PreTrain_Net_MutualEncodingMode(n_field_info, n_baseF, num_heads, num_layers, num_fields=len(Old_field_names)).to(device)
    ...
    # Load the P2F net
    P2F_net = Finetuning_SensorToFeatures_MutualEncoding(layer_sizes, Final_layer_sizes, PreTrained_net).to(device)
    ...
    with open('data_split/data_split_NO.pic', 'rb') as fp: 
    ...
    if (NET_TYPE == 0):
        net = UNSeen_FineTune_Net(N_selected, n_field_info, n_baseF, PreTrained_net).to(device)
    else:
