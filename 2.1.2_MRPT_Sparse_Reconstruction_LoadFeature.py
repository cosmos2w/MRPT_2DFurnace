...
if __name__ == '__main__':
    ...
    if (NET_TYPE == 0):
        ...
        net = Mutual_SensorToFeature_InterInference_LoadFeature(layer_sizes, PreTrained_net).to(device)
    ...
    for epoch in range(N_EPOCH):
        ...
        for U, Y, G, Yin, Gin in get_data_iter(U_train, Y_train, G_train, Yin_train, Gin_train):
            ...
            #----------------------------
            if (NET_TYPE == 0):
                Predicted_Features, Unified_Feature_output = net(Yin, Gin, num_heads, Min_Value_Train, Max_Value_Train)
            elif (NET_TYPE == 1):
            ...
            mse_loss_Unifed_train = criterion(Unified_Feature_output, LR_Unified_train)
            ...
            if EVALUATE is True and (epoch + 1) % N_REPORT == 0:
                with torch.no_grad():
                    ...
                    DeNormalized_train_output = Denormalize_data(Unified_Feature_output, Min_Value_Train[len(field_names)], Max_Value_Train[len(field_names)]) # This is the recovered "Global_Unified_U"
        ...
        with torch.no_grad():  
            for U, Y, G, Yin, Gin in get_data_iter(U_test, Y_test, G_test, Yin_test, Gin_test):
                ...
                mse_loss_Unifed_test = criterion(Unified_Feature_output, LR_Unified_test)
                test_losses[ len(field_names) ] = mse_loss_Unifed_test
                test_loss += mse_loss_Unifed_test

                if EVALUATE is True and (epoch + 1) % N_REPORT == 0:
                    baseF = PreTrained_net.PosNet(Y)
                    DeNormalized_test_output = Denormalize_data(Unified_Feature_output, Min_Value_Train[len(field_names)], Max_Value_Train[len(field_names)]) # This is the recovered "Global_Unified_U"
